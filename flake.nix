      ...
      overlayFor = getStdenv: final: prev:
        let currentStdenv = getStdenv final; in
        {
          ...
          in currentStdenv.mkDerivation (finalAttrs: {
            ...
            BOOST_LIBRARYDIR = "${builtins.placeholder "out"}/lib";
            BOOST_ROOT = "${lib.getDev pkgs.boost}";

            AWS_SDK_CPP_INCLUDEDIR = "${lib.getDev (pkgs.aws-sdk-cpp.override {
                apis = ["s3" "transfer"];
                customMemoryManagement = false;
            })}/include";
            AWS_SDK_CPP_LIBRARYDIR = "${lib.getLib (pkgs.aws-sdk-cpp.override {
                apis = ["s3" "transfer"];
                customMemoryManagement = false;
            })}/lib";
            ...
            mesonFlags = mesonFlags 
              ++ [ "-Dsysconfdir=$(out)/etc" "-DPRECOMPILE_HEADERS=true" ]
              # ++ ["-Daws_sdk_cpp_lib_dir=$AWS_SDK_LIBRARYDIR" "-Daws_sdk_cpp_include_dir=$AWS_SDK_INCLUDEDIR" ]
              ++ lib.optional stdenv.hostPlatform.isStatic "-Denable-embedded-sandbox-shell"
              # ++ [ (lib.enableFeature finalAttrs.doCheck "tests") ]
              ++ lib.optionals finalAttrs.doCheck testMesonFlags
